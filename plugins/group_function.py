...
########################################################################################################################
    ...
    if m.text == (get_db_botname() or "لوتس"):
        texting = ["اؤمر " + (get_db_botname() or "لوتس") + " شتريد؟❤️🥺",
                   "اى يقلب " + (get_db_botname() or "لوتس") + "❤️",
                   ...
                   "قلبى بينادى على " + (get_db_botname() or "لوتس") + "😘",
                   "نعسان محدش يصحينى🙄"
                   ]
        await m.reply_text(random.choice(texting))
        return

    if m.text == "بوت غادر" or m.text == (get_db_botname() or "لوتس") + " غادر":
        if secsudo(m):
            try:
...
########################################################################################################################
    ...
    if m.text == "السورس" or m.text == "سورس" or m.text == "يا سورس":
        ...
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            ...
            [ InlineKeyboardButton ( "𝐒 𝐎 𝐔 𝐑 𝐂 𝐄", url=f"https://t.me/l2_2Y" )], 
        [InlineKeyboardButton("اضف البوت الي مجموعتك ✅",
                              url=f"https://t.me/{get_bot_information () [1]}?startgroup=new")],
        ])
        await m.reply_text("""
╭──── • ◈ • ────╮
么 [𝐒 𝐎 𝐔 𝐑 𝐂 𝐄 ](t.me/l2_2Y)
么 [𝐆 𝐑 𝐎 𝐏](t.me/C7_7M)
么 [𝐕 𝐎 𝐃 𝐈 𝐊 𝐀](t.me/YwYvYe)
╰──── • ◈ • ────╯
